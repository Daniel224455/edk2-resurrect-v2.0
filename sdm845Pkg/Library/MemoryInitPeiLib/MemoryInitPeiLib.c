...
MemoryPeim(IN EFI_PHYSICAL_ADDRESS UefiMemoryBase, IN UINT64 UefiMemorySize)
{
  ...
  DEBUG((EFI_D_INFO, "RUN THROUGH EACH MEM_DESCRIPTOR...\n"));
  DEBUG((EFI_D_INFO, "CONFIG FOUND: %d GB RAM\n", MemGB));
  DEBUG((EFI_D_INFO, "MEMORY SET, START BOOTING EDK2...\n"));
  DEBUG((EFI_D_INFO, "START PRINTING LOGS FROM HERE. HAND OVER TO MMU!\n"));
